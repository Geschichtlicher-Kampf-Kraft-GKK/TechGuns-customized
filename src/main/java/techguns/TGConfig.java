...
public class TGConfig {
	...
	public static int spawnWeightArmySoldier;
	public static int spawnWeightCyberDemon;
	public static int spawnWeightPsychoSteve;
	public static int spawnWeightCommando;
	public static int spawnWeightStormTrooper;
	public static int spawnWeightOutcast;
	public static int spawnWeightDictatorDave;
	public static int spawnWeightAttackHelicopter;
	...
	public static void initValues() {
		...
		machinesNeedNoPower = config.getBoolean("machinesNeedNoPower", config.CATEGORY_GENERAL, true, "Machines don't need power, activate this if you don't want to install a mod with generators and still be able to use the machines");
		...
		spawnWeightTGOverworld = config.getInt("Techguns Spawnweight Overworld", "NPC Spawn", 10000, 0, 10000, "Spawn weigth of Techguns NPCs, determines how many TG npcs spawn");
		spawnWeightTGNether = config.getInt("Techguns Spawnweight NEther", "NPC Spawn", 10000, 0, 10000, "Spawn weigth of Techguns NPCs in the Nether, determines how many TG npcs spawn");
		
		spawnWeightZombieSoldier  = config.getInt("SpawnWeightZombieSoldier", "NPC Spawn", 0, 0, 10000, "Spawn weight for spawning Zombie Soldiers");
		spawnWeightZombieFarmer  = config.getInt("SpawnWeightZombieFarmer", "NPC Spawn", 0, 0, 10000, "Spawn weight for spawning Zombie Farmers");
		spawnWeightZombieMiner = config.getInt("SpawnWeightZombieMiner", "NPC Spawn", 0, 0, 10000, "Spawn weight for spawning Zombie Miners");
		spawnWeightZombiePigmanSoldier  = config.getInt("SpawnWeightZombiePigmanSoldier", "NPC Spawn", 0, 0, 10000, "Spawn weight for spawning Zombie Pigman Soldiers");
		spawnWeightSkeletonSoldier = config.getInt("SpawnWeightSkeletonSoldier", "NPC Spawn", 0, 0, 10000, "Spawn weight for spawning Skeleton Soldiers");


		spawnWeightBandit = config.getInt("SpawnWeightBandit", "NPC Spawn", 8000, 0, 10000, "spawn Weight changed. [0 - 10000]");
		spawnWeightArmySoldier = config.getInt("SpawnWeightArmySoldier", "NPC Spawn", 500, 0, 10000, "spawn Weight changed. [0 - 10000]");
		spawnWeightCyberDemon = config.getInt("SpawnWeightCyberDemon", "NPC Spawn", 500, 0, 10000, "spawn Weight changed. [0 - 10000]");
		spawnWeightPsychoSteve = config.getInt("SpawnWeightPsychoSteve", "NPC Spawn", 200, 0, 10000, "spawn Weight changed. [0 - 10000]");
		spawnWeightCommando = config.getInt("SpawnWeightCommando", "NPC Spawn", 200, 0, 10000, "spawn Weight changed. [0 - 10000]");
		spawnWeightStormTrooper = config.getInt("SpawnWeightStormTrooper", "NPC Spawn", 200, 0, 10000, "spawn Weight changed. [0 - 10000]");
		spawnWeightOutcast = config.getInt("SpawnWeightOutcast", "NPC Spawn", 200, 0, 10000, "spawn Weight changed. [0 - 10000]");
		spawnWeightDictatorDave = config.getInt("SpawnWeightDictatorDave", "NPC Spawn", 60, 0, 10000, "spawn Weight changed. [0 - 10000]");
		spawnWeightAttackHelicopter = config.getInt("SpawnWeightAttackHelicopter", "NPC Spawn", 50, 0, 10000, "spawn Weight changed. [0 - 10000]");
		spawnWeightSuperMutantBasic = config.getInt("SpawnWeightSuperMutantBasic", "NPC Spawn", 40, 0, 10000, "spawn Weight changed. [0 - 10000]");
		spawnWeightSuperMutantElite = config.getInt("SpawnWeightSuperMutantElite", "NPC Spawn", 30, 0, 10000, "spawn Weight changed. [0 - 10000]");
		spawnWeightSuperMutantHeavy = config.getInt("SpawnWeightSuperMutantHeavy", "NPC Spawn", 20, 0, 10000, "spawn Weight changed. [0 - 10000]");
		...
		damagePvP = config.getFloat("DamagePvP", DAMAGE_FACTORS, 1.0f, 0.0f, 100.0f, "Damage factor Techguns weapons deal when fired from players against other players, is zero when PvP is disabled");
		
		damageTurretToPlayer = config.getFloat("DamageTurretToPlayer", DAMAGE_FACTORS, 1.0f, 0.0f, 100.0f, "Damage factor Techguns Turrets deal when hitting players");
		...
		doOreGenTitanium = config.getBoolean("doOreGenTitanium", WORLDGEN, false, "Generate Titanium, not generated by most mods mods, leave it on in most cases");

		doOreGenUranium = config.getBoolean("doOreGenUranium", WORLDGEN, false, "Generate Uranium, disable if other mod already adds it and you want only 1 type. OreDictEntry:'oreUranium' ");
		
		doWorldspawn = config.getBoolean("SpawnStructures", WORLDGEN, false, "Should Structures (military bases) be spawned in the world?");
		...
		spawnOreClusterStructures = config.getBoolean("SpawnOreClusterStructures", WORLDGEN, false, "When worldgen is enabled, include structure spawns that contain ore clusters.");
		
		explosiveChargeMaxBlockHardness = config.getFloat("ExplosiveChargeMaxHardness", config.CATEGORY_GENERAL, 30, 0.0f, Float.MAX_VALUE, "Highest blockHardness normal explosive charges can break, obsidian is 50.0)");
		
		explosiveChargeAdvancedMaxBlockHardness = config.getFloat("ExplosiveChargeAdvancedMaxHardness", config.CATEGORY_GENERAL, Float.MAX_VALUE, 0.0f, Float.MAX_VALUE, "Highest blockHardness advanced explosive charges can break, obsidian is 50.0)");
