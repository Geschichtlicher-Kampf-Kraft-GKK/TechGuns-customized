...
public class TGSpawnManager {
	...
	public static void handleSpawn(World w, Entity dummyEnt){
		...
		if (totalweight>0){
			...
			if (chosenSpawn!=null){
				...
				try {
					if(chosenSpawn.ifFly) {
						GenericFlyingMob entNew = chosenSpawn.fly.getDeclaredConstructor(World.class).newInstance(w);
						setPositionsAndReplace(w, entNew, dummyEnt, danger);
					}else{
						GenericNPC entNew = chosenSpawn.type.getDeclaredConstructor(World.class).newInstance(w);
						setPositionsAndReplace(w, entNew, dummyEnt, danger);
					}
	...
	private static void setPositionsAndReplace(World w, GenericFlyingMob newNpc, Entity entity, int difficulty){
		newNpc.setLocationAndAngles(entity.posX, entity.posY, entity.posZ, entity.rotationYaw, 0.0f);
		//newNpc.setRotationYawHead(entity.rotationYaw);
		//newNpc.onSpawnWithEgg((IEntityLivingData)null);
		w.spawnEntity(newNpc);
