...
public class GrinderRecipes {
	...
	public static void addRecipesWithChance(ItemStack input, HashMap<ItemStack, Double>... hashMaps){
		HashMap<ItemStack, Double> maps = new HashMap<>();
		Arrays.stream(hashMaps).forEach(maps::putAll);
		ItemStack[] items = new ItemStack[maps.size()];
		double[] chances = new double[maps.size()];
		int i = 0; for(ItemStack item: maps.keySet()){ items[i++] = item; }
		i = 0; for(double chance: maps.values()){ chances[i++] = chance; }
		recipes.add(new GrinderRecipeChance(new ItemStackOreDict(input, input.getCount()), items, chances));
	}

	public static void addRecipesWithChance(GenericGun gun, HashMap<ItemStack, Double>... hashMaps){
		HashMap<ItemStack, Double> maps = new HashMap<>();
		Arrays.stream(hashMaps).forEach(maps::putAll);
		ItemStack[] items = new ItemStack[maps.size()];
		double[] chances = new double[maps.size()];
